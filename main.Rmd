...
We will start with the whole dataset `data_clean` and by training a classifier on `success` starting with a logistic regression model

```{r entire set, eval=F, include=F}
glm_clean <- glm(success~. -country_txt -provstate -city -targsubtype1_txt -gname -natlty1_txt, data_clean, family='binomial')
summary(glm_clean)
Anova(glm_clean)
```
...
```{r, eval=F, echo=F}
set.seed(1)
Y <- data_clean[,'success']
X <- model.matrix(data=data_clean, success~. -country_txt - provstate - city - targsubtype1_txt - natlty1_txt - gname)[,-1]

lasso_clean <- cv.glmnet(x=X, y=Y, alpha=1, nfold=10, family='binomial')
plot(lasso_clean)
...
Let us take a look at the differences between countries, namely Iraq, Japan, USA, and the larger set of countries identified as developed. We will build a linear regression model and run Anova() to remove (backward select out) categories that are not significant at the $\alpha=0.01$ level.

Starting with Japan, we first remove `nwound` before modling building as it would be highly correlated with `nikill`. In reality, we can't really use `nwound` to predict `nkill` as they occur at the same time (it makes sense that more wounded implies more killed, and vice versa). We are interested in deeper relations, if they exist. The summaries can be found in the appendix.
...
Adjusted R-Squared for Japan: `r summary(lm_japan)$adj.r.squared`

RSE for Japan: `r summary(lm_japan)$sigma`


Our backwards selected model is still very large (`r nrow(coef(summary(lm_japan)))` predictors still remain). Let us try LASSO (cross-validated) to see if we can reduce the complexity of our model. Below is our graph of cross-validated mean errors. Our final model's code is shown in the appendix.
...
Let us also try running random forest model on this datset. We must remove  `provstate`, `city`, `targsubtype_txt`, `natlty1_txt`, and `gname` as they have oveer 53 predictors. We tuned our random forest appropriatly with `mtry` and `ntree`. Displayed is the MSE of our RF:
...
The samme approaches were applied the USA, Syria, and Iraq dataset. Their outputs are shown in the appendix and the overall summaries are:

```{r usa, include=F}
lm_usa <- lm(data=data_usa, nkill~.- nwound - provstate - city - targsubtype1_txt - natlty1_txt - gname) 
alpha = 0.01
lm1_usa_anova <- Anova(lm_usa)
lm1_usa_anova <- lm1_usa_anova[!is.na(lm1_usa_anova[,4]),]
while (TRUE) {
  if (max(lm1_usa_anova[,4]) > alpha) {
    lm1_usa_anova <- lm1_usa_anova[-which.max(lm1_usa_anova[,4]),]
  } else {
    break
  }
}
...
```{r, include= F}
input <- as.formula(paste('nkill ~', paste(rownames(lm1_usa_anova), collapse = ' + ')))

lm_usa <- lm(input, data=data_usa)
```

Adjusted R-Squared for usa: `r summary(lm_usa)$adj.r.squared`

RSE for usa: `r summary(lm_usa)$sigma`
...
Adjusted R-Squared for usa (LASSO): `r summary(lasso_usa)$adj.r.squared`

RSE for usa (LASSO): `r summary(lasso_usa)$sigma`
...
MSE at 150 trees: `r rf_usa$mse[150]`


```{r syria, include=F}
lm_syria <- lm(data=data_syria, nkill~.- nwound - provstate - city - targsubtype1_txt - natlty1_txt - gname) 
alpha = 0.01
lm1_syria_anova <- Anova(lm_syria)
lm1_syria_anova <- lm1_syria_anova[!is.na(lm1_syria_anova[,4]),]
while (TRUE) {
  if (max(lm1_syria_anova[,4]) > alpha) {
    lm1_syria_anova <- lm1_syria_anova[-which.max(lm1_syria_anova[,4]),]
  } else {
    break
  }
}
...
```{r, include= F}
input <- as.formula(paste('nkill ~', paste(rownames(lm1_syria_anova), collapse = ' + ')))

lm_syria <- lm(input, data=data_syria)
```

Adjusted R-Squared for syria: `r summary(lm_syria)$adj.r.squared`

RSE for syria: `r summary(lm_syria)$sigma`
...
Adjusted R-Squared for syria (LASSO): `r summary(lasso_syria)$adj.r.squared`

RSE for syria (LASSO): `r summary(lasso_syria)$sigma`
...
MSE at 150 trees: `r rf_syria$mse[150]`


```{r iraq, include=F}
lm_iraq <- lm(data=data_iraq, nkill~.- nwound - provstate - city - targsubtype1_txt - natlty1_txt - gname) 
alpha = 0.01
lm1_iraq_anova <- Anova(lm_iraq)
lm1_iraq_anova <- lm1_iraq_anova[!is.na(lm1_iraq_anova[,4]),]
while (TRUE) {
  if (max(lm1_iraq_anova[,4]) > alpha) {
    lm1_iraq_anova <- lm1_iraq_anova[-which.max(lm1_iraq_anova[,4]),]
  } else {
    break
  }
}
...
```{r, include= F}
input <- as.formula(paste('nkill ~', paste(rownames(lm1_iraq_anova), collapse = ' + ')))

lm_iraq <- lm(input, data=data_iraq)
```

Adjusted R-Squared for iraq: `r summary(lm_iraq)$adj.r.squared`

RSE for iraq: `r summary(lm_iraq)$sigma`
...
Adjusted R-Squared for iraq (LASSO): `r summary(lasso_iraq)$adj.r.squared`

RSE for iraq (LASSO): `r summary(lasso_iraq)$sigma`

```{r, include=F}
# rf_iraq
set.seed(1)
#lol doesn't run too large still
rf_iraq <- randomForest(data=data_iraq, nkill~.- nwound - provstate - city - targsubtype1_txt - weapsubtype1_txt - natlty1_txt - gname, mtry=3, ntree=50)
...
MSE at 150 trees: `r rf_iraq$mse[150]`
